
namespace ZC_GPA_Calculator;

internal static class Grades
{
    public static readonly double A = 4.0;
    public static readonly double A_MINUS = 3.7;
    public static readonly double B_PLUS = 3.3;
    public static readonly double B = 3.0;
    public static readonly double B_MINUS = 2.7;
    public static readonly double C_PLUS = 2.3;
    public static readonly double C = 2.0;
    public static readonly double C_MINUS = 1.7;
    public static readonly double D_PLUS = 1.3;
    public static readonly double D = 1.0;
    public static readonly double F = 0.0;
}

// 2023 Fall and later
internal static class Grades2
{
	public static readonly double A = 4.0;
	public static readonly double A_MINUS = 3.6667;
	public static readonly double B_PLUS = 3.3333;
	public static readonly double B = 3.0;
	public static readonly double B_MINUS = 2.6667;
	public static readonly double C_PLUS = 2.3333;
	public static readonly double C = 2.0;
	public static readonly double C_MINUS = 1.6667;
	public static readonly double D_PLUS = 1.3333;
	public static readonly double D = 1.0;
	public static readonly double F = 0.0;
}